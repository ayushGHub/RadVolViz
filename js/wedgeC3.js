...
var heatmap = d3.heatmap()
             //.std(183)
	           .dx(0.4)
             .dy(0.4);
var heatmapData = heatmap(newarr);
//console.log(newarr);
//console.log(heatmapData);
//console.log(heatmapData[1][1][1]);
var pp=0;
var hT = [];
for (var i = 0; i < heatmapData.length; i++) {
  ...
  hist2d = d3.hist2d()
    ...
    var TT = 0;
     ...
     var checkD = guiControls.binValue;

    for (var i = 0; i < heatmapData.length; i++) {

      if(heatmapData[i].length != 0){

      if(heatmapData[i].length > 500)
      {
          var PP = [];
        while(PP.length < 5*Math.log2(heatmapData[i].length)){
          var rd = Math.floor(Math.random()*heatmapData[i].length) + 1;
          if(PP.indexOf(rd) > -1) continue;
          PP[PP.length] = rd;
          QQ.push(heatmapData[i][rd-1]);
        }
      }

      else if (heatmapData[i].length < 500 & heatmapData[i].length > 50) {
        var PP = [];
        while(PP.length < Math.log2(heatmapData[i].length)){
          var rd = Math.floor(Math.random()*heatmapData[i].length) + 1;
          if(PP.indexOf(rd) > -1) continue;
          PP[PP.length] = rd;
          ...
          var PP = [];
          while(PP.length < heatmapData[i].length/2){
          var rd = Math.floor(Math.random()*heatmapData[i].length) + 1;
          if(PP.indexOf(rd) > -1) continue;
          PP[PP.length] = rd;
          QQ.push(heatmapData[i][rd-1]);
  ...
  for (var i = 0; i < hist.length; i++) {
    if(hist[i].length > 500)
    {
      ...
      //  var rd=1;
        ...
        rd = Math.floor(random()*hist[i].length) + 1;
        if(P.indexOf(rd) > -1) continue;
        P[P.length] = rd;
        Q.push(hist[i][rd-1]);
      }
